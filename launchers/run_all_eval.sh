...
if [ -z "${1:-}" ]; then
    echo "Usage: $0 models|all|<CONFIG_DIR> [OUTPUT_PREFIX=eval/eval-] [BATCH_SIZE=16] [MAX_SAMPLES=None] [MODELS_DIR=configs/1-trirex/TESTS]"
    echo "       models -> iterate training configs (default: configs/1-trirex/TESTS) and eval on: trirex-bite, trirex, simple-questions, web-qsp"
...
MODE="$MODE_OR_DIR"
if [ "$MODE" = "all" ]; then
    # Explicit list of supported eval configs
    CONFIG_FILES=(
        "configs/4-eval/eval_bite.yaml"         # trirex-bite
        "configs/4-eval/eval_trirex.yaml"       # trirex
        "configs/4-eval/eval_simpleQA.yaml"     # simple-questions
        "configs/4-eval/eval_web_qsp.yaml"      # web-qsp
    )
...
accelerate launch --config-file configs/accelerate_evalconfig.yaml \
  evaluate.py --config "\${CONFIG_FILE}" --output_file "\${OUTPUT_FILE}" --batch_size "\${BATCH_SIZE}" --max_samples "\${MAX_SAMPLES}" --no_baseline
EOF
    chmod +x "$JOB_SCRIPT"
    sbatch "$JOB_SCRIPT"
}
...
if [ "$MODE" = "models" ]; then
    # Sweep over training configs and evaluate on all datasets
    CLI_MODELS_DIR="${5:-}"
    if [ -n "$CLI_MODELS_DIR" ]; then
        MODELS_DIR="$CLI_MODELS_DIR"
    else
    ...
    for TRAIN_CFG in "${TRAIN_CONFIGS[@]}"; do
        ...
        for DATASET in "${!DATASET_TEMPLATES[@]}"; do
    ...
    echo "All evaluation jobs submitted for models (${#TRAIN_CONFIGS[@]} models x ${#DATASET_TEMPLATES[@]} datasets)."
else
